...
const HELP = {
  de: {
    ...
    transfers: '<p>Die Erzeugung muss dem Bedarf nicht exakt entsprechen, da Energie mit Nachbarländern ausgetauscht und in Speichersystemen zwischen- oder eingelagert werden kann.</p><p>Deutschland ist über zahlreiche Grenzkuppelstellen mit den Übertragungsnetzen seiner Nachbarländer verbunden. Als Teil des kontinentaleuropäischen Verbundnetzes fließt Strom dabei meist über mehrere parallele Leitungen, nicht über eine einzelne benannte Verbindung.</p><p>Positive Werte bedeuten einen Nettoimport nach Deutschland, negative Werte einen Nettoexport.</p>',
  ...
  en: {
    ...
    transfers: "<p>Generation doesn't need to match demand exactly, as energy can be exchanged with neighbouring countries and moved into or out of storage systems.</p><p>Germany is linked to its neighbours' transmission networks through numerous cross-border connection points. As part of the Continental European synchronous grid, power mostly flows across several parallel lines rather than a single named cable.</p><p>Positive values mean a net import into Germany, negative values a net export.</p>",
