...
**The forecast** is the day-ahead one for solar, both winds and demand, and exists because the measured mix is always an hour or so behind: a quarter hour must end before the operators can report it. [Prediction](classes/State/Prediction.php) estimates the quarter hours that have happened but have not been published, drawn as a dashed line — inside a band whose width is the error measured for that line at that reach, once the delay passes an hour. Everything is anchored to the last confirmed quarter hour, with the forecast supplying only the *change* since then: solar and wind move by their forecasts, demand by its forecast with weights that shift with the season, coal and gas take three tenths of whatever the demand asks for beyond what solar and wind give, and the borders take the rest, so demand = generation + transfers holds of every estimated quarter hour. The day-ahead forecast rather than the one revised through the day, because the revised one failed in exactly the stalls the estimate exists for, and because the day-ahead one is never rewritten afterwards, so it could be measured over every stored quarter hour rather than guessed at. The estimates are never written to the database, so no confirmed figure can be displaced by one.
