...
const HELP = {
  de: {
    time: '<p>Erzeugungsdaten werden alle fünfzehn Minuten aktualisiert, entsprechend der Auflösung der zugrundeliegenden Marktdaten.</p>',
    ...
    emissions: '<p>Die Verbrennung von Braunkohle, Steinkohle, Erdgas und Biomasse setzt Kohlendioxid frei. Dieser Wert schätzt die CO₂-Intensität der deutschen Stromerzeugung auf Basis des aktuellen Erzeugungsmixes.</p>',
    ...
    transfers: '<p>Die Erzeugung muss dem Bedarf nicht exakt entsprechen, da Strom mit den Nachbarländern gehandelt und in Speichersystemen zwischen- oder eingelagert werden kann.</p><p>Gezeigt wird der kommerzielle Außenhandel: die für den jeweiligen Zeitraum vereinbarten Stromlieferungen zwischen Deutschland und seinen Nachbarn. Weil Deutschland Teil des kontinentaleuropäischen Verbundnetzes ist, fließt der Strom physikalisch meist über mehrere parallele Leitungen und nicht über eine einzelne benannte Verbindung, sodass die tatsächlichen physikalischen Flüsse von den gehandelten Mengen abweichen können.</p><p>Positive Werte bedeuten einen Import nach Deutschland, negative Werte einen Export.</p>',
  ...
  en: {
    time: '<p>Generation data is updated every fifteen minutes, matching the resolution of the underlying market data.</p>',
    ...
    emissions: '<p>Burning lignite, hard coal, gas, and biomass produces carbon dioxide. This figure estimates the carbon intensity of German electricity generation from the current generation mix.</p>',
    ...
    transfers: "<p>Generation doesn't need to match demand exactly, as electricity can be traded with neighbouring countries and moved into or out of storage systems.</p><p>What's shown is commercial cross-border trade: the electricity deliveries agreed between Germany and its neighbours for each period. Because Germany is part of the Continental European synchronous grid, the power physically flows across several parallel lines rather than a single named cable, so actual physical flows can differ from the traded volumes.</p><p>Positive values mean an import into Germany, negative values an export.</p>",
